...
<div class="pageHeader">
  <form rel="pagerForm" method="post" action="view/criteria/partnerLookup" onsubmit="return dwzSearch(this, 'dialog');">
    <div class="searchBar">
      ...
      <div class="subBar">
        <ul>
          ...
          <li><a class="button" href="javascript:$.bringBack({partnerId:0,partnerName:'取消'})" title="查找带回"><span>取消选择</span></a></li>
        </ul>
      </div>
    </div>
  </form>
        ...
        <tbody>
          <c:forEach items="${requestScope.result.data}" var="op" varStatus="var">
            <tr id="${op.id }" hasChild="true" target="sid_op" rel="${op.id}">
                ...
                <td>${op.name}</td>
                <td>
                    <a class="btnSelect" href="javascript:$.bringBack({partnerId:${op.id}, partnerName:'${op.name}'})" title="查找带回">选择</a>
                </td>
            </tr>
          </c:forEach>
        </tbody>
