...
<div class="pageHeader">
  <form rel="pagerForm" method="post" action="view/app/listLookup" onsubmit="return dwzSearch(this, 'dialog');">
    <div class="searchBar">
        ...
        <tbody>
          <c:forEach items="${requestScope.result.data}" var="op" varStatus="var">
            <tr id="${op.id }" hasChild="true" target="sid_op" rel="${op.id}">
                ...
                <td>${op.name}</td>
                <td>
                    <a class="btnSelect" href="javascript:$.bringBack({id:${op.id}, name:'${op.name}'})" title="查找带回">选择</a>
                </td>
            </tr>
          </c:forEach>
        </tbody>
